## VERBATIM COPY of C:\Users\aaron\gyro-squadron-45\scripts\tilt_source.gd
## (spec/jam_spec.json law "injectable_input": jam-legal pre-existing code).
## Everything below the provenance banner is byte-identical to the source.
## The Feel constants it references live in this project's scripts/feel.gd.
## SLIME LINE default mode is MODE_TOUCH — slug.gd sets it in _init() — and on
## mobile MODE_DEVICE tilt runs through the exact same shaping pipeline.
